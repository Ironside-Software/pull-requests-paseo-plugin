...
const details = { repository: "acme/widget", number: 42, title: "Improve workspace navigation", url: "https://github.com/acme/widget/pull/42", state: "open", draft: false, author: "alice", login: "alice", head: "feature", base: "main", headSha: "a".repeat(40), body: "Example description", updatedAt: "2026-09-22", files: 1, commits: 1, additions: 3, deletions: 1, reviewers: [], assignees: [], labels: [], mergeable: true, mergeState: "clean", canMerge: false, canReview: false, mergeMethods: [] };
...
test("PR list shares workspace lookup and launches independently of opening PR details", async () => {
  const cache = new QueryClient();
  let root!: ReactTestRenderer;
  listItems = [42, 43].map(number => ({ ...details, id: number, number, title: `PR ${number}`, url: `https://github.com/acme/widget/pull/${number}`, authored: true, assigned: false }));
